...
class Queries:
	...
	def badgeQueries(self, group_name):
		...
		self.queries = { 'welcome' :
							{ 'experimental' : 'SELECT user_id, ba_date FROM th_up_badges_awarded WHERE user_id != 0 AND b_id = 1 AND bap_ns = 3 AND ba_date > "2013-01-31" AND not_badge = 0 AND user_id IN (SELECT rev_user FROM th_up_profiles WHERE DATE(post_date) > "2013-01-31")',
							 'control' : 'SELECT DISTINCT rev_user, DATE(post_date) FROM th_up_profiles AS p, (SELECT user_id, ba_date FROM th_up_badges_awarded WHERE user_id != 0 AND b_id = 1 AND bap_ns = 3 AND ba_date >= "2013-01-24" AND not_badge = 0 AND user_id IN (SELECT rev_user FROM th_up_profiles WHERE DATE(post_date) >= "2013-01-24")) AS tmp WHERE p.rev_user != tmp.user_id AND p.rev_user != 0 AND DATE(p.post_date) >= "2013-01-24" ORDER BY RAND() LIMIT 228',
...
# 					{ 'activity' : 'SELECT count(rev_id) AS cum_revs FROM enwiki.revision WHERE rev_user = %s AND DATE(DATE_FORMAT(rev_timestamp,"%s")) > "%s" AND rev_page = 34745517' }
# 						}
# 					}
# 		qry = self.queries[group_name]
# 		return qry
