...
def getUserData(cursor):
		cursor.execute('UPDATE th_up_badges_awarded as t, enwiki.user AS u SET t.user_id = u.user_id, t.user_name = u.user_name, t.user_registration = u.user_registration WHERE REPLACE(t.bap_title,"_"," ") = u.user_name AND t.user_id IS NULL')
		conn.commit()
